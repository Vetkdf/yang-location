...
export class UserinfoComponent extends BaseComponent  { 
  ...
  constructor( @Inject('auxiliary') public auxiliary, @Inject('business_system') private business_system, private route: ActivatedRoute) {
    super(auxiliary, '/business_system', '/userinfo');
  }
  ...
  bindpage(event: number): void {
    ...
    this.business_system.GetListPagingByUser(pageindex, ConstantsList.pageSize, this.comId, this.getSendToken()).then(backobj => {
  ...
  ngOnInit() {
    ...
    this.business_system.GetListUserGroup(this.getSendToken()).
      then(backobj => {
